...
The plugin registers one model-facing tool, `penglai_office_extract`. It resolves the user-supplied path through `ctx.fs`, performs a bounded raw-byte read, checks the ZIP central directory before decompression, and returns capped plain text. It does not execute macros, write Office files, access the network, or bypass DSH with Node's raw filesystem API.
